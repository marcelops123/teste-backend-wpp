    ...
    try {
        ...
        .then(() => {
            if(media.filename.includes('ogg')){
                logger_1.logger.info('Arquivo OGG salvo com sucesso!');
          ...
          .then(() => {
            logger_1.logger.info('Arquivo salvo com sucesso.');
          })
          .catch((err) => {
            logger_1.logger.error('Ocorreu um erro:', err);
          }); 
    }
    catch (err) {
...
const handleMessage = (msg, wbot) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const groupConfig = process.env.GROUPS;
    if (chat.isGroup === true && groupConfig === 'off'){
        return;
    }
    try {
        let msgContact;
